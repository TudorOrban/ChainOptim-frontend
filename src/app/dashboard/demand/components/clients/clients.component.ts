...
export class ClientsComponent implements OnInit {
    ...
    loadClients(organizationId: number) {
        this.fallbackManagerService.updateLoading(true);

        this.clientService
            .getClientsByOrganizationIdAdvanced(organizationId, this.searchQuery, this.currentSortOption.value, this.ascending, this.page, this.itemsPerPage)
            .subscribe({
                next: (paginatedResults) => {
                    // Get results and count
                    this.clients = paginatedResults.results;
    ...
    handleSearch(query: string): void {
        if (this.searchQuery !== query) {
            ...
            this.loadClients(this.currentOrganization!.id);
        }
    }
